...
const transform = {
  ...
  B: bucket => Math.floor(bucket / 2) * 15
};
...
function removeFreebies(buckets) {
  ...
  console.log('freeBs2', currentBs);
  const newBs = currentBs - freeBs;
  console.log('freeBs3', newBs);
  const array = newBs <= 0 ? [] : 'B'.repeat(newBs).split('');
  console.log('freeBs3', array);
  return {
    ...buckets,
    B: array,
  };
...
module.exports = function (skusText) {
  ...
  const removedFreeItems = removeFreebies(buckets);
  console.log(removedFreeItems);

  const discounts = Object
    .entries(removedFreeItems)
